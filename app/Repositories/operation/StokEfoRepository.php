...
use App\Http\Resources\operation\StokEfoResource;
use App\Models\operation\Cog;
use App\Models\operation\StokEfo;

class StokEfoRepository
{
    public function getAll($request, $id_kontraktor)
    {
        ...
        $stok_efo = StokEfo::query();

        $stok_efo->with(['toStokEfoDetail', 'toDomEfo']);

        $stok_efo->whereBetween('date_in', [$start_date, $end_date]);

        // filter id_kontraktor
        if ($id_kontraktor != null) {
            $stok_efo->whereKontraktor($id_kontraktor);
        }

        // filter id_kontraktor
        if ($request->id_kontraktor) {
            $stok_efo->whereKontraktor($request->id_kontraktor);
        }
        ...
        $items = StokEfoResource::collection($data)->toArray($request);
