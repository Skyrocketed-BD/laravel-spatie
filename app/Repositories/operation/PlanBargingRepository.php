...
use App\Http\Resources\operation\PlanBargingResource;
use App\Models\operation\Cog;
use App\Models\operation\PlanBarging;

class PlanBargingRepository
{
    public function getAll($request, $id_kontraktor)
    {
        ...
        $plan_barging = PlanBarging::query();

        $plan_barging->with(['toPlanBargingDetail', 'toShippingInstruction', 'toInvoiceFob.toTransaction.toTransactionTerm']);

        $plan_barging->whereBetween('date', [$start_date, $end_date]);

        // filter id_kontraktor
        if ($id_kontraktor != null) {
            $plan_barging->whereKontraktor($id_kontraktor);
        }

        // filter id_kontraktor
        if ($request->id_kontraktor) {
            $plan_barging->whereKontraktor($request->id_kontraktor);
        }

        $plan_barging->latest();

        $data = $plan_barging->get();

        $items = PlanBargingResource::collection($data)->toArray($request);
