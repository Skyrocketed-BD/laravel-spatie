...
use App\Http\Resources\MenuBodyResource;
...
class MenuBodyController extends Controller
{
    ...
    {
        $data = MenuBody::with(['toMenuPermission.toPermission'])->whereIdMenuCategory($id_menu_category)->orderBy('position')->get();

        return Response::json(MenuBodyResource::collection($data), 200);
    }
    ...
    public function store(Request $request)
    {
        DB::beginTransaction();
        try {
            $data = MenuBody::create([
                'id_menu_category' => $request->id_menu_category,
                'name'             => $request->name,
                'icon'             => $request->icon,
                'url'              => $request->url,
