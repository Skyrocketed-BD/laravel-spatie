...
class MenuModuleController extends Controller
{
    ...
    public function store(MenuModuleRequest $request)
    {
        DB::beginTransaction();
        try {
            $data = MenuModule::create([
                'name' => $request->name
            ]);

            DB::commit();
            ActivityLogHelper::log('admin:create_module_menu', 1, ['name' => $request->name]);

            return ApiResponseClass::sendResponse($data, 'Menu Module Created Successfully');
        } catch (\Exception $e) {
            ActivityLogHelper::log('admin:create_module_menu', 0, ['error' => $e]);
    ...
    public function update(MenuModuleRequest $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            ActivityLogHelper::log('admin:update_module_menu', 1, ['name' => $request->name]);

            return ApiResponseClass::sendResponse($data, 'Menu Module Updated Successfully');
        } catch (\Exception $e) {
            ActivityLogHelper::log('admin:update_module_menu', 0, ['error' => $e]);
            return ApiResponseClass::rollback($e);
        }
    }
    ...
    public function destroy($id)
    {
        try {
            $data = MenuModule::find($id);
            $data->delete();
            ActivityLogHelper::log('admin:delete_module_menu', 1, ['name' => $data->name]);

            return ApiResponseClass::sendResponse($data, 'Menu Module Deleted Successfully');
        } catch (\Exception $e) {
            ActivityLogHelper::log('admin:delete_module_menu', 0, ['error' => $e]);
